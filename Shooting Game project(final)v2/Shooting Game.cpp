...
public:
	...
	virtual void update() {
		...
		sprintf(bullet, "%d / %d", bullet_i, bullet_m); sprintf(bullet_s, "BOOM : %d", bullet_sb); sprintf(totalscore_a, "Total Score : %3.0f ::: Exit : ESC Key",totalscore);
		...
		for (auto b : entities) {
			if ((a->name == "asteroid" || a->name == "asteroid_s") && (b->name == "bullet" || b->name == "s_bullet"))
				if (isCollide(a, b))
				{
					...
					a->life = false;
					if(b->name != "s_bullet") b->life = false;
	...
	int nCmdShow)
{
	...
	Animation sintro(0, 0, 1600, 900, 1, 0);
	Animation soutro(0, 0, 1600, 900, 1, 0);
	...
	Animation sPlayer(45, 0, 83, 51, 1, 0);
	Animation sPlayer_go(0, 2, 38, 51, 1, 0);
	...
	RECT rect_in;
	SetRect(&rect_in, SCREEN_WIDTH * 1 / 2-170, SCREEN_HEIGHT * 3 / 4, SCREEN_WIDTH * 1 / 2+15, SCREEN_HEIGHT * 3 / 4);

	Entity *out = new Entity();
	out->settings(soutro, outro, 0, 0);
	RECT rect_out;
	SetRect(&rect_out, SCREEN_WIDTH * 1 / 2 - 70, SCREEN_HEIGHT * 3 / 4, SCREEN_WIDTH * 1 / 2 + 15, SCREEN_HEIGHT * 3 / 4);
	...
	while (TRUE)
	{
		...
		if (STAGE == 0) {
			...
			in->anim.update();
			in->draw();
			Font->DrawTextA(d3dspt, "Start Game : Press Space Key!", -1, &rect_in, DT_NOCLIP, D3DCOLOR_XRGB(255, 255, 255));
