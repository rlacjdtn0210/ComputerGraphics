...
void init_game(void)
{
	...
	{
		enemy[i].init((float)(rand() % 300 + 700), rand() % 500-100);
	}
...
void do_game_logic(void)
{
	...
	for (int i = 0; i < ENEMY_NUM; i++)
	{
		if (enemy[i].x_pos < -500)
			enemy[i].init((float)(rand() % 500 + 700), rand() % 500 - 100);
		else
			enemy[i].move();
	...
	for (int i = 0; i < BULLET_NUM; i++) {
		if (bullet[i].show() == false)
		{
			...
			{
				bullet[i].active();
				bullet[i].init(hero.x_pos+100, hero.y_pos+100);
			}
		}

		if (bullet[i].show() == true)
		{
			if (bullet[i].x_pos > 700)
				bullet[i].hide();
			else
				bullet[i].move();
		}
